...
import (
	...
	"github.com/laplacenetwork/gochain/rpc/internal/jsonrpc"
...
type Client interface {
	...
	GetBlockByNumber(number uint64) (val *Block, err error)
...
func (client *clientImpl) GetBlockByNumber(number uint64) (val *Block, err error) {
...
func (client *clientImpl) BalanceOfAsset(address string, asset string, decimals int) (*fixed.Number, error) {
	...
	return fixed.FromHex(valstr, decimals)
}
...
func (client *clientImpl) DecimalsOfAsset(asset string) (int, error) {
	...
	val, err := fixed.FromHex(valstr, 0)
...
func (client *clientImpl) SuggestGasPrice() (*fixed.Number, error) {
	...
	err := client.Call2("eth_gasPrice", &val, "latest")
	if err != nil {
		return nil, err
	}

	return fixed.FromHex(val, 18)
}
